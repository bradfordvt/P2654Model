#!/usr/bin/env python
"""
    Driver interface to the P2654Simulations simulation framework simulating various UUT designs.
    Copyright (C) 2019 Bradford G. Van Treuren

    This code contains the API interfaces to the P2654Simulations project framework to provide the
    access to the virtual controller devices on a virtual Wishbone bus.  Supported are the direct devices
    Telnet client to the P2654Simulations Telnet server, a virtual ATE interface class for access to the P
    virtual Wishbone bus of the P2654Simulations framework, a virtual JTAGController interface to the
    first JTAG controller device, a virtual JTAGCnontroller2 interface to the second JTAG controller device,
    a virtual I2CController to an I2C host device, a virtual SPIController to a SPI host device, and a
    GPIOController interface class to a virtual GPIO controller device.
...
__copyright__ = "Copyright 2019, VT Enterprises Consulting Services"
...
# from hdl.hosts.jtaghost.JTAG_Ctrl_Master import SHIFT_DR, SHIFT_IR, RUN_TEST_IDLE, TEST_LOGIC_RESET
# from hdl.hosts.jtaghost.tapsim import *
# The following states imported from hdl.hosts.jtaghost.JTAG_Ctrl_Master of P2654Simulations project:
...
# The following state imported from hdl.hosts.jtaghost.tappsim of P2654Simulations project:
...
class JTAGController:
    ...
    def ba_scan_ir(self, tdi_vector, count, start=SHIFT_IR, end=RUN_TEST_IDLE):
        """
        ...
        :param start: State to start the scan/shift operations
        :param end: State to end up in following the current scan/shift operation (must be a stable state)
    ...
    def ba_scan_dr(self, tdi_vector, count, start=SHIFT_DR, end=RUN_TEST_IDLE):
        """
    ...
    def scan_ir(self, count, tdi_string, start=SHIFT_IR, end=RUN_TEST_IDLE):
        """
        ...
        if len(tdi_vector) > 1:
            tdi_vector.reverse()
            # print("tdi_vector = ", tdi_vector)
        tdo_vector = self.ba_scan_ir(tdi_vector, count, start, end)
        # print("tdo_vector = ", tdo_vector)
        if len(tdo_vector) > 1:
            tdo_vector.reverse()
    ...
    def scan_dr(self, count, tdi_string, start=SHIFT_DR, end=RUN_TEST_IDLE):
        """
        ...
        if len(tdi_vector) > 1:
            tdi_vector.reverse()
            # print("tdi_vector = ", tdi_vector)
        tdo_vector = self.ba_scan_dr(tdi_vector, count, start, end)
        # print("tdo_vector = ", tdo_vector)
        if len(tdo_vector) > 1:
            tdo_vector.reverse()
        tdo_string = tdo_vector.hex().upper()
    ...
    def softreset(self):
        start = TEST_LOGIC_RESET
        end = TEST_LOGIC_RESET
        self.__set_bit_count(5)
        self.__set_state_start(start)
        self.__set_state_end(end)
        self.__set_control_register(0x1)  # Start the scan
        status = self.__get_status_register()
...
class JTAGController2:
    ...
    def ba_scan_ir(self, tdi_vector, count, start=SI_SHIFT_IR, end=SI_RUN_TEST_IDLE):
        """
        Scan the vector to the TAP with the IR data and capture the response in tdo_vector
        :param tdi_vector: Data to be shifted out as bytearray
        :param count: number of bits to shift
    ...
    def ba_scan_dr(self, tdi_vector, count, start=SI_SHIFT_DR, end=SI_RUN_TEST_IDLE):
        """
        Scan the vector to the TAP with the DR data and capture the response in tdo_vector
        :param ate_inst:
        :param tdi_vector: Data to be shifted out as bytearray
    ...
    def scan_ir(self, count, tdi_string, start=SI_SHIFT_IR, end=SI_RUN_TEST_IDLE):
        """
    ...
    def scan_dr(self, count, tdi_string, start=SI_SHIFT_DR, end=SI_RUN_TEST_IDLE):
        """
    ...
    def softreset(self):
        start = SI_TEST_LOGIC_RESET
        end = SI_TEST_LOGIC_RESET
        self.__set_chain_length(5)
        self.__set_state_start(start)
        self.__set_state_end(end)
        self.__set_command(SCAN)
        self.__set_control_register(0x1)  # Start the scan
